...
function init(){
    ...
    moduleListArr.map(function(curModule){
        ...
        //     //创建详情页
        //     var markdownPath = Path.join(paths["source"], curModule["name"], "readme.md");
        //     if(FileObj.isExit(markdownPath) == false){
        //         markdownPath = Path.join(paths["source"], curModule["name"], "README.md");
        //     }

        //     var markdown = FileObj.read(markdownPath);
...
function create(curModule){
    ...
    var moduleJSON = Parse.module(moduleName, fileJSON, curModule);                
    ...
    dll.download(libList, function(){
        ...
        if(fs.existsSync(nodeModulesPath)){                
            for(var libName in libList){
                ...
                if(mainPath){
                    mainPath = Path.join(libName, mainPath);
                }                
        ...
        //
        dll.create(moduleName, libList, (libArr)=>{
            if(libArr.length > 0){
