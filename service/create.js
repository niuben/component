...
function createDetailPage(json) {
    ...
    files.map(function(item, index){
        ...
        // 创建
        function createFileMenu(fileArr){
            var tpl = "";
            for(var i = 0; i < fileArr.length; i++){
                var fileName = fileArr[i]["name"];
                var className = "";
                if(typeof fileName == "string" && fileName.toLocaleLowerCase() == "readme.md"){
                    className = "active";
                }
                tpl += "<li class='"+ className +"'>";
                tpl += fileName;
                if(fileArr[i]["files"] !== undefined) {
                    tpl += "<ul>";
                    tpl += createFileMenu(fileArr[i]["files"]);
                    tpl += "</ul>";
                }
                tpl += "</li>";
            }
